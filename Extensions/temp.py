...
class Temp:
    def __init__(self, ctx: "AppContext"):
        ...
        Temp_frame = tk.Button(
            self.main.win, text="Temp", command=self.Temp_tab,
    ...
    def Temp_tab(self):
        if "frame_temp" not in self.tab.frames or not self.tab.frames["frame_temp"].winfo_exists():
            frame_temp = tk.Frame(
                self.main.win, bg=self.CFG.bg)
            frame_temp.place(x=0, y=20, width=910, height=490)
            
            self.tab.frames.append(frame_temp)
            self.tab.frame_dict["temp"] = frame_temp
        
        temp_label = tk.Label(
            frame_temp, text="Temp", bg=self.CFG.bg,
            fg=self.CFG.fg, font=("arial", 12))
        temp_label.pack()
        ...
        log_temp.place(x=0, y=30, width=900, height=405)
